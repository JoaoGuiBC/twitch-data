...
interface UserFollowedStreamsInterface {
  id: string;
...
interface UserFollowedStreamsFormatted extends UserFollowedStreamsInterface {
  user_avatar_url: string;
}

export function Home() {
  const [topGames, setTopGames] = useState<TopGamesInterface[]>([]);
  ...
  async function getUserFollowedStreamsAvatar(
    userFollowedStreamsData: UserFollowedStreamsInterface[]
  ) {
    return Promise.all(
  ...
  async function getUserFollowedStreams() {
    try {
      const response = await api.get<{ data: UserFollowedStreamsInterface[] }>(
        ...
        <FlatList
          data={
            !isLoadingTopGames
              ? topGames
              : [
                  { id: '1' } as TopGamesInterface,
                  { id: '2' } as TopGamesInterface,
                  { id: '3' } as TopGamesInterface,
                ]
          }
          keyExtractor={(item) => item.id}
          horizontal
